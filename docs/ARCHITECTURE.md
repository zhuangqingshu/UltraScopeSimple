...
十四个面板堆在一列里总高约 1740px，720 高的窗口只看得到前六个，下面的静默消失。
改为 `ttk.Notebook` 四页：Capture / Channels / Trigger / Analysis，最高的一页 613px。

两处不是纯装饰的决定：

- **`App.TABS` 同时是布局表和面板清单**。`self.panels`（`_set_enabled` 遍历它）由它生成，
  所以漏填一个面板不会得到"面板在但位置不对"，而是"面板被构造了却从未 grid"——
  看起来和没写这个面板一模一样。`tests/test_gui_layout.py` 扫描 App 上所有 `Panel`
  实例与 `self.panels` 比对，专门盯这个
- **每页是一个 `ScrollableColumn`**，装不下时才出现滚动条。分页只解决"此刻放得下",
  滚动才解决"以后再加一个面板会不会重蹈覆辙"。Tk 没有可滚动 Frame，这里是常规的
  Canvas + create_window 构造；滚轮必须 `bind_all` 才能在子控件上收到，因此每个 column
  都会收到每一次滚动并自行判断指针是否在自己身上——**波形图上的滚轮是微调触发电平**，
  抢掉它就破坏了既有交互

状态栏放在 Notebook 之外：命令失败的提示不该被藏在刚切走的那一页上。
