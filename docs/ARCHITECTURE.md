...
- [ ] **接真机验证 CLI**：默认导出、`--single`、`--mode raw --memdepth long`、`--acquire average`、`--measure`、`--plot`
- [ ] **接真机验证 GUI**：连接/断开、live 刷新、Run/Stop/Auto/Single/Force、通道与时基与触发改设、深存储采集、CSV/PNG 导出、关窗时面板控制权交还
- [ ] 真机通过后再处理第 9 节遗留的三个行为缺陷

最后两项必须接真实示波器——本项目没有硬件模拟，自动化测试只覆盖到仪器层以下。GUI 冒烟测试只证明控件能建起来、显示路径不报错，**不能证明 SCPI 交互正确**。
