...
- `analysis.py` — local measurement over a captured `Waveform` (cursor readings, interpolated sampling, FFT spectrum, trace parameters, channel arithmetic, XY pairs). Pure functions, no SCPI, so these features work while disconnected. Channel maths is deliberately done here rather than via `:MATH:OPER`: numpy is more flexible and it adds no command spelling to verify. `measurements_of()` takes a bare trace so a computed one is measured by the same code as a channel.
