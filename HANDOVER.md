...
- **纵轴单位有四种**（`Spectrum.values(scale)`）：`dBVrms` 默认 / `Vrms` / `dBVpk` / `V`。
  前两个与仪器 FFT 菜单的 Scale 一致（User's Guide 第 44 页）。原先算峰值幅度却标成
  `dBV` 是个**标注错误**——dBV 按惯例指相对 1 V<sub>rms</sub>，读数偏高 3.01 dB。

  **每 bin 的 RMS 不是一律除以 √2。** 直流不是正弦，其 RMS 就是自身；偶数长度记录的
  Nyquist bin 同理（它在两个值之间交替，不描出正弦）。这两个除错了帕塞瓦尔等式就不成立，
  整体 Vrms 会算低 25%。`Spectrum.rms` 里这两行例外看着像特例，其实是定义要求的，
  `test_the_bins_account_for_the_whole_signal` 直接拿帕塞瓦尔当判据盯着它
...
| **FFT 的 RMS 换算** | 整条波形的 Vrms 三条独立算路吻合：仪器 `:MEAS:VRMS?` 报 1.94 V，本地时域 1.94224 V，FFT 各 bin 方均根和 1.94224 V（帕塞瓦尔）|
